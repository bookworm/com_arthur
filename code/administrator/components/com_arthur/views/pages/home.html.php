...
$checks = array(
	'resourcesWritable' => function() use ($notify) 
	{
		...
		$path     = str_replace(dirname(ARTHUR_APP_PATH) . '/', null, $path);
		$solution = null;
	...
	{
		$template = $self->html->link('template', 'http://lithify.me/docs/arthur/template');
	...
	{
		$routing = $self->html->link('routing', 'http://lithify.me/docs/arthur/net/http/Router');
	...
	'tests' => function() use ($notify, $self) 
	{
		$tests = $self->html->link('run all tests', array(
			'controller' => 'arthur\test\Controller',
			'args' => 'all'
		));
		$dashboard = $self->html->link('test dashboard', array('controller' => 'arthur\test\Controller'));
		$ticket = $self->html->link('file a ticket', 'https://github.com/UnionOfRAD/arthur/issues');

		return $notify(
			'notice',
			'Run the tests.',
			"Check the builtin {$dashboard} or {$tests} now to ensure Arthur
...
<ul class="additional-resources">
	<li>
		...
		is a guide for PHP users who are looking to get a good idea of what Arthur can do.
		The guide is part of the official Arthur manual,
		<?php echo $this->html->link('The Definitive Guide', 'http://lithify.me/docs/manual'); ?>.
	</li>
	<li>
		The <?php echo $this->html->link('API documentation', 'http://lithify.me/docs/arthur'); ?>
		has all the implementation details you've been looking for.
	</li>
	<li>
		Chat with other Arthur users and the team developing Arthur.
	...
	<li>
		Browse the Arthur
		<?php echo $this->html->link('Repository', 'https://github.com/UnionOfRAD/arthur'); ?>
		or read the
		<?php echo $this->html->link('Wiki', 'https://github.com/UnionOfRAD/arthur/wiki'); ?>.
	</li>
</ul>